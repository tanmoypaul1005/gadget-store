        ...
        <ul className="flex flex-col">
          {
            categoryData?.data?.slice(0, 10)?.map((item, index) => (
              <li key={index} className="relative sidebar men_nav_item">
                ...
                <ul className="absolute flex-col items-start justify-start hidden gap-2 p-4 font-normal text-black bg-white border rounded shadow-lg hoveredItems w-52 top-10 right-[-210px]">
                  {
                    item?.child?.length > 0 ? (
